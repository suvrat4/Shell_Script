...
for idx, row_data in enumerate(all_issue_data, start=2):
...
# Create a new tab for MTTR statistics
ws2 = wb.create_sheet('MTTR Statistics')
ws2['A1'] = 'MTTR (Mean Time to Resolve) Statistics'
ws2['A2'] = 'Average MTTR (hours)'
ws2['B2'] = mttr_avg
ws2['A3'] = 'Median MTTR (hours)'
ws2['B3'] = mttr_median

# Calculate counts for each status
status_counts = df['Status'].value_counts()

# Create a new tab for status counts and chart
ws3 = wb.create_sheet('Status Counts')
ws3['A1'] = 'Status'
ws3['B1'] = 'Count'

for idx, (status, count) in enumerate(status_counts.items(), start=2):
...
data = Reference(ws3, min_col=2, min_row=1, max_col=2, max_row=len(status_counts))
categories = Reference(ws3, min_col=1, min_row=2, max_row=len(status_counts))
